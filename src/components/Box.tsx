    ...
    return (
        <>
            <div className='grid-item' onClick={() => setOpen(true)}>
                ...
                <span className='role-text'>{props.type !== "project" && info[props.tag].title}</span>
            </div>
            <Modal
                open={open}
            ...
            >
                <div className={`grid-item back ${props.type}`} onClick={() => setOpen(false)}style={{
                    ...
                    <div className="title-name-box">
                        {<span className="title-back">{info[props.tag].title}</span>}
                        {<span className="name-back">{info[props.tag].name}</span>}
                    </div>
                    <div className='date-location-box'>
                        {<span className="date-back">{info[props.tag].date}</span>}
                        {info[props.tag].location && <span className="location-back"> | {info[props.tag].location}</span>}
                    </div>
                    <div className='info-image-box'>
                        {props.type !== "education" && <div className='paragraph-box'>
                            {<span className="paragraph-back">{info[props.tag].p1}</span>}
                            {<span className="paragraph-back">{info[props.tag].p2}</span>}
                            {<span className="paragraph-back">{info[props.tag].p3}</span>}
                            {<span className="paragraph-back">{info[props.tag].p4}</span>}
                        </div>}
                        {props.type === "education" && <div className='paragraph-box'>
                            <h2 className="edu-section">Academics</h2>
                            {<span className="paragraph-back"><ul>
                            ...
                            {<span className="paragraph-back"><ul>{info[props.tag].p2}</ul></span>}
                            ...
                            <ul>
                                <li>
                                    ...
                                    {<span className="paragraph-back"><ul className="course-list">
